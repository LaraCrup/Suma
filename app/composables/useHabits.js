export const useHabits = () => {
    ...
    const countCompletedLogsInPeriod = (habitId) => async (start, end) => {
        const { data: periodLogs } = await client
            .from('habit_logs')
            .select('id')
            .eq('habit_id', habitId)
            ...
            .lte('date', end)
            .eq('completed', true)
        return periodLogs?.length || 0
    }

    const shouldShowHabitForDate = async (habit, dateStr) =>
        await shouldShowHabitForDateWith(habit, dateStr, countCompletedLogsInPeriod(habit.id))

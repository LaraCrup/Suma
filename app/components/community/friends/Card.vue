    ...
    >
        <div class="flex gap-3 items-center">
            <div class="w-6 2xl:w-8 h-6 2xl:h-8 rounded-full overflow-hidden flex items-center justify-center bg-green-light flex-shrink-0">
                ...
                <span v-else class="text-[0.5rem] 2xl:text-sm text-light font-bold">{{ friend?.display_name?.charAt(0).toUpperCase() }}</span>
            </div>
            <div class="flex items-center gap-1">
                <p class="text-xs 2xl:text-sm">{{ friend?.display_name }}</p>

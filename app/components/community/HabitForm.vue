...
const submitLabel = computed(() => {
    if (props.isLoading) return props.isEditing ? 'Guardando...' : 'Creando...'
    return props.isEditing ? 'Guardar cambios' : 'Crear comunidad'
})
...
const validateForm = () => {
    ...
    if (formData.frequencyVariant && formData.frequencyVariant.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().includes('especificos')) {
        const match = formData.frequencyVariant.match(/\(([^)]+)\)/)
        if (!match || !match[1]?.trim()) {
            errors.value.frequencyVariant = 'Por favor seleccioná los días específicos'
            return false
        }
    }
    return true
...
const handleSubmit = () => {
    ...
    emit('submit', {
        ...
        identity: formData.habitIdentity.trim(),
        unit: formData.habitUnit.trim(),
...
onMounted(() => loadFormData(props.initialData))

watch(() => props.initialData, (newData) => loadFormData(newData), { deep: true })
</script>

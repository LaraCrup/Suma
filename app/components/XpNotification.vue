        ...
        <div v-if="store.visible && store.current"
            ...
            <div class="bg-green-dark p-3 flex items-center gap-3">
                <div class="w-6 h-6 flex justify-center items-center rounded-full border-gray border-[1px]">
                    <NuxtImg src="/images/brillo.svg"
                        alt="Brillo" class="w-3" />
                </div>
                <div class="flex flex-col">
                    <span
                        class="font-montserrat font-bold text-lg leading-tight"
                        :class="isRevoke ? 'text-error' : 'text-accent'"
                    >
                        {{ isRevoke ? '-' : '+' }} {{ Math.abs(store.current.xpAmount) }} XP
                    </span>
...
const REVOKE_LABELS = {
    habit_completed: 'Hábito descompletado',
    all_habits_daily: 'Ya no completaste todos',
    weekly_goal_met: 'Meta semanal perdida',
    community_habit_completed: 'Hábito comunitario revertido',
    friend_added: 'Amigo eliminado',
}

const isRevoke = computed(() => (store.current?.xpAmount ?? 0) < 0)
const label = computed(() => {
    const key = store.current?.actionKey
    return isRevoke.value
        ? (REVOKE_LABELS[key] ?? 'XP revocado')
        : (ACTION_LABELS[key] ?? '¡XP obtenido!')
})

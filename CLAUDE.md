...
- **El XP de `join_community` se sella en la BD, no en localStorage (jul-2026)**: hasta ahora `recordCommunityJoin` guardaba `joined_community_{userId}_{communityId}` en localStorage, así que el XP no estaba atado a la **membresía** sino a "este navegador nunca abrió esta página". Consecuencia: abrir por primera vez una comunidad de la que sos miembro hace semanas pagaba +15, y cualquier cosa que vacíe el localStorage (limpiar datos del sitio, incógnito, otro navegador, otra compu, la PWA instalada vs. Safari en iOS) reseteaba el guard de **todas** las comunidades de una y las volvía a pagar. Ahora el guard es la columna `community_members.join_xp_granted_at` y lo reclama el RPC `claim_community_join_xp(p_community_id)` de [20260731_community_join_xp.sql](supabase/migrations/20260731_community_join_xp.sql).
  - **Por qué RPC y no un `update` del cliente**: `community_members` no tiene policy de `UPDATE` a propósito (ver la nota de `leave_community` más abajo), así que el sellado necesita `SECURITY DEFINER`.
  - **La atomicidad la da el `where join_xp_granted_at is null`**: el `row_count` del UPDATE decide quién cobra, así que dos pestañas (o dos dispositivos) en paralelo pagan una sola vez.
  - **El creador sella pero no cobra** (ya cobró `create_community`), y quien no es miembro no matchea ninguna fila: antes, visitar la página sin ser miembro igual pagaba.
  - **La migración hace backfill**: marca todas las membresías existentes como ya-otorgadas. Sin eso, la primera visita post-deploy dispararía el bug una última vez para todo el mundo. Las membresías nuevas entran con `null` y cobran normalmente.
  - **Orden de despliegue**: primero la migración, después el cliente. Al revés, `recordCommunityJoin` llama a un RPC inexistente y nadie cobra el XP.
...
- **`NavigationBackArrow` siempre lleva `url` (jul-2026)**: la prop es `required` y **ningún** uso puede depender del `router.go(-1)` implícito. El historial no es la jerarquía de la app: llegar a una pantalla por un push, por un deep link o por un `navigateTo` post-submit hacía que la flecha volviera a cualquier lado (o saliera de la app). Cada uso pasa el **padre jerárquico**, no "la anterior". Los tres casos con más de un punto de entrada:
  - **`CommunityHeader` recibe `backUrl` por prop**, porque lo comparten dos páginas con padres distintos: `/comunidades/[id]` vuelve a `/comunidades` y `/comunidades/[id]/habito` vuelve a `/comunidades/[id]`.
  - **`/usuarios/[id]` se llega desde tres lados** (`/comunidades`, `/amigos`, `/comunidades/[id]/detalle`), así que `CommunityFriendsCard` y `CommunityFriendsCardAdd` le adosan `?from={route.fullPath}` al link, y la página vuelve a ese `from`. Se valida que empiece con `/` y no con `//` (evita redirigir afuera) y cae a `/comunidades` si falta. Si agregás otro link a `/usuarios/[id]`, pasá el `from`.
  - **El wizard de comunidad va estrictamente hacia atrás**: paso 1 → `/comunidades`, paso 2 → paso 1, paso 3 → paso 2. El paso 1 vuelve **siempre** a `/comunidades`, incluso si se llegó ahí desde el link "Agregar" del paso 2: que la flecha llevara al paso 2 (o sea, hacia adelante) se sentía roto. El paso 2 vuelve al paso 1 con el estado **vivo** de sus refs y el paso 3 con el de la query, así que ir para atrás y volver con "Siguiente" no pierde nada; lo único que se pierde es salir del wizard desde el paso 1, que es la intención.
- **El scroll lo maneja `<main>`, no el `window`**: [main.css](app/assets/css/main.css) le pone `overflow-y: auto` a `main` dentro del grid, así que el `scrollBehavior` de Vue Router (que toca el scroll del documento) **no hace nada** y al cambiar de página se conservaba el scroll de la anterior. Lo resetea un `watch` sobre `route.path` en [DefaultMain](app/components/default/Main.vue) que hace `mainRef.scrollTop = 0`. Es sobre `path` y **no** sobre `fullPath` a propósito: los cambios de solo query (`?date=`, `?from=`) no deben saltar al tope.
...
| `join_community` | 15 | Usuario abre por primera vez una comunidad a la que lo sumaron (guard en `community_members.join_xp_granted_at`, vía el RPC `claim_community_join_xp` — ver §14) | — |
